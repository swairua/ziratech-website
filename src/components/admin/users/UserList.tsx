...
export const UserList = ({ searchQuery, roleFilter, statusFilter, onEditUser }: UserListProps) => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      // Build query with joins to get user roles
      let query = supabase
        .from('profiles')
        .select(`
          id,
      ...
      // Transform data and apply role filter
      let transformedUsers = data?.map(user => ({
        id: user.user_id,
      ...
      // Apply role filter
      if (roleFilter !== 'all') {
        transformedUsers = transformedUsers.filter(user => user.role === roleFilter);
      }
